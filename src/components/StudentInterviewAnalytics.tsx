...
const StudentInterviewAnalytics = ({ studentId }: { studentId: string }) => {
  ...
  useEffect(() => {
    const fetchAnalysisData = async () => {
      try {
        setLoading(true);
        // In a real implementation, you would fetch data for the specific student
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {categoryPerformanceData.length > 0 
                ? categoryPerformanceData.reduce((prev, current) => 
                    (prev.score > current.score) ? prev : current
                  ).name
                : 'N/A'}
            </div>
            <p className="text-xs text-muted-foreground">Your strongest skill</p>
          </CardContent>
        </Card>
      </div>
...
export default StudentInterviewAnalytics;