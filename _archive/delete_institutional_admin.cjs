...
async function deleteInstitutionalAdmin() {
  try {
    console.log('Deleting institutional admin user...');
    
    // The user ID from the previous check
    const userId = 'h5prJwoIdfewPxaBr5aPFUXqJ0n2';
    // The institution ID
    ...
    console.log(`Successfully deleted institutional admin user ${userId} from institution ${institutionId}`);
    ...
    console.error('Error deleting institutional admin user:', error);
  } finally {
    // Clean up
